...
public:
    serial_node(std::string port, unsigned int baud_rate) : io(), serial(io,port) //     SimpleSerial(std::string port, unsigned int baud_rate) : io(), serial(io,port)
    {
      ...
      Transmit_Sub_ = node_.subscribe<std_msgs::String>(Toppic_For_Data_To_Be_Transmitted, 100, &serial_node::Serial_Transmit_Callback, this);
...
private:
    ...
    void Serial_Transmit_Callback(const std_msgs::String::ConstPtr& msg);
};
